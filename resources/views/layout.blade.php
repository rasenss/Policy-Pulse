...
<body class="text-slate-600 antialiased flex flex-col min-h-screen">
    ...
    <nav x-data="{ scrolled: false, mobileOpen: false }" 
        ...
        <div x-show="mobileOpen" 
            ...
            <div class="mt-2 border-t border-slate-100 pt-4 px-2 flex items-center gap-2 text-xs font-bold text-emerald-600">
                <span class="w-2 h-2 rounded-full bg-emerald-500"></span> System Online
            </div>
        </div>
    </nav>

    <main class="flex-grow container mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-28 md:pt-32 page-animate">
        @yield('content')
    </main>
    ...
    <footer class="bg-white border-t border-slate-100 mt-auto py-8 relative z-10" data-turbo-permanent id="footer">
        <div class="max-w-7xl mx-auto px-4 text-center">
            ...
            <div class="mt-3 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-slate-50 border border-slate-100 shadow-sm">
                <span class="text-xs text-slate-500 font-mono">Data: <span class="font-bold text-blue-600">{{ $totalTweets ?? '...' }}</span> Tweets</span>
            </div>
        </div>
    </footer>
