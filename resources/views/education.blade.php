...
<div class="max-w-4xl mx-auto" x-data="quizApp()">
    ...
    <div x-show="state === 'start'" class="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 p-16 text-center relative overflow-hidden hover-scale">
        ...
        <p class="text-slate-500 mb-10">Terdiri dari 40 Pilihan Ganda & 10 Essay Analisis.<br>Siapkan argumen logis Anda.</p>
    ...
    <div x-show="state === 'quiz'" class="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 overflow-hidden min-h-[500px]" style="display: none;">
        <!-- Header Quiz -->
        <div class="bg-white/90 backdrop-blur px-10 py-6 border-b border-slate-100 flex justify-between items-center sticky top-0 z-10">
        ...
        <div class="p-10">
            ...
            <div class="mb-6">
                <span class="inline-flex items-center px-3 py-1 rounded-lg text-xs font-bold tracking-wide uppercase shadow-sm border"
                    ...
                    <span x-text="questions[currentIdx]?.type === 'essay' ? '✍️ Soal Essay' : '📝 Pilihan Ganda'"></span>
                </span>
            </div>
            ...
            <div x-show="questions[currentIdx]?.type === 'essay'">
                <textarea x-model="essayAnswer" 
                    class="w-full p-5 rounded-2xl border-2 border-slate-200 focus:border-purple-500 focus:ring-0 text-slate-700 leading-relaxed h-40 resize-none shadow-inner" 
